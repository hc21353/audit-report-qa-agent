...
def structured_query(
    ...
    for r in results[:3]:
        h2 = r.get('section_h2') or '?'
        h3 = str(r.get('section_h3') or '?')[:40]
        print(f"  year={r.get('year')}, h2={h2}, h3={h3}", flush=True)

    PREVIEW_LIMIT = 2000  # 미리보기 길이 (이전: 1000)
    ...
    for r in results:
        ...
        entry = {
            ...
            "csv_refs": csv_re.findall(full_content),
        }
        output.append(entry)
