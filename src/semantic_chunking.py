...
def main():
    base_dir = Path(__file__).parent.parent / "parsed_data" / "output"
    all_chunks = []
    
    for year_dir in sorted(base_dir.iterdir()):
        if year_dir.is_dir() and year_dir.name.startswith("감사보고서_"):
            print(f"Processing {year_dir.name}...")
            year_chunks = process_year(year_dir)
            all_chunks.extend(year_chunks)
            
    output_file = Path(__file__).parent.parent / "parsed_data" / "chunks" / "semantic_chunks_tagged.jsonl"
