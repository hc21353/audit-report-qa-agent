...
{{
  ...
  "structured_queries": [
    {{
      ...
      "section_h2": "DB 구조에서 실제 존재하는 h2 (없으면 빈 문자열)",
      "section_h3": "DB 구조에서 실제 존재하는 h3 앞부분 (없으면 빈 문자열)",
      "section_h4": "",
      "keyword": "본문 추가 필터 키워드 (없으면 빈 문자열)",
      "content_type": "결정 규칙에 따라 설정",
      "limit": 20
    }}
  ]
}}
...
## structured_queries content_type 결정 규칙
- 재무 수치/표 데이터 (총자산, 영업이익, 유동자산, 부채, 자본 등):
  → content_type = "all"
- 텍스트 설명 질문 (감사의견, 핵심감사사항, 회계정책, 주석 설명 등):
  → content_type = "text"
- 불확실:
  → content_type = "all"
