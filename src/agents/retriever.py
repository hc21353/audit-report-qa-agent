...
1. hybrid_search
   ...
   - 필수 파라미터: query (str) — 반드시 있어야 함. "keyword" 필드 없음.
   - 선택 파라미터: top_k (int, 기본 10), year (int, 기본 0=전체), section_h2 (str), mode ("hybrid"|"vector"|"bm25")
...
2. structured_query
   ...
   - 파라미터:
     ...
     section_h2 (str): 대분류 (정확 매칭) — 반드시 실제 존재하는 값 사용
     section_h3 (str): 하위 섹션 (부분 매칭)
     section_h4, section_h5 (str): 더 세부 항목
...
4. get_full_content
   ...
   - 파라미터: chunk_ids (str, 쉼표 구분), include_adjacent (bool, 기본 false)"""
...
(아래 섹션명은 실제 DB에서 조회한 값입니다. structured_query 사용 시 이 이름만 사용하세요.)
{live_sections}

## 검색 전략 가이드
- section_h2/h3는 위 "실제 DB 섹션 구조"에 있는 값만 사용 (임의로 만들지 말 것)
...
def retriever_node(state: dict, tools: dict, llm=None, db_context: str = "") -> dict:
    ...
    for i, r in enumerate(final[:5]):
        ...
        print(
            ...
            f"section={r.get('section_path','?')[:60]} {src}",
            flush=True,
        )
...
def _generate_search_plan(llm, query: str, intent: str, years: list,
                          semantic_queries: list, live_sections: str = "") -> dict:
    """LLM에게 시맨틱 검색 계획을 요청"""
    context = f"""## 사용자 질문
{query}
...
def _generate_refine_plan(llm, query: str, results: list, live_sections: str = "") -> dict:
    """결과 부족 시 추가 검색 계획"""
