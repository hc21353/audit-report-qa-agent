...
def retriever_node(state: dict, tools: dict, llm=None) -> dict:
    """
...
{json.dumps(rewritten_queries, ensure_ascii=False)}"""
    ...
    prompt = PLANNING_PROMPT.format(tool_descriptions=TOOL_DESCRIPTIONS) + "\n\n" + context

    try:
        response = llm.invoke([
            SystemMessage(content="당신은 검색 계획 수립 전문가입니다. JSON만 반환하세요."),
...
def _generate_refine_plan(llm, query: str, results: list) -> dict:
    """결과 부족 시 추가 검색 계획 생성"""
