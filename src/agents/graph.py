...
def build_graph(config: Config, db=None, vector_store=None) -> StateGraph:
    ...
    # vector_store는 (collection, embedder) 튜플 또는 None
    if vector_store is not None and isinstance(vector_store, tuple):
        collection, embedder = vector_store
    else:
        collection, embedder = None, None

    init_hybrid_search(collection, embedder, db)
    if db:
...
if __name__ == "__main__":
    ...
    try:
        ...
        vector_store = (collection, embedder)
        print(f"[CLI] ChromaDB loaded: {collection.count()} vectors from {vector_dir}")
