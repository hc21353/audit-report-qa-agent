...
def prepare_vector_store(config: Config, db: AuditDB, run_cfg: dict):
    """임베딩 + 청킹 설정에 맞는 벡터스토어 로드/빌드"""
    embedding_name = run_cfg.get("embedding", config.active_embedding)
    chunking_strategy = run_cfg.get("chunking", config.active_chunking)
    backend_name = run_cfg.get("vector_store", "faiss")

    print(f"  [VectorStore] embedding={embedding_name}, chunking={chunking_strategy}, backend={backend_name}")

    # app.yaml에서 active_strategy 임시 교체
    original_chunking = config.app.get("chunking", {}).get("active_strategy")
    original_embedding = config.app.get("embedding", {}).get("active_model")

    config.app["chunking"]["active_strategy"] = chunking_strategy
    config.app["embedding"]["active_model"] = embedding_name
    ...
    try:
        ...
        # 원상복구
        config.app["chunking"]["active_strategy"] = original_chunking
        config.app["embedding"]["active_model"] = original_embedding
